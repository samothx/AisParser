...
var printByte = function printByte(_byte) {
  var bits = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 8;
  return Number(_byte & (1 << bits) - 1).toString(2).padStart(bits, '0');
};
var AisBitField = exports["default"] = /*#__PURE__*/function () {
  ...
  return _createClass(AisBitField, [{
    key: "bits",
    get: function get() {
      return this._bits;
    }
    ...
    value: function _getByte(idx) {
      var _byte2 = this._bytes[idx];
      if (typeof _byte2 === 'number') {
        return _byte2;
      } else {
        var _char = this._aisStr.charCodeAt(idx);
        if (_char > 47) {
          if (_char < 88) {
            return this._bytes[idx] = _char - 48;
          } else {
            if (_char < 96) {
              throw MOD_NAME + '.parse() invalid character encountered:' + _char + ' at index ' + idx;
            } else {
              if (_char < 120) {
                return this._bytes[idx] = _char - 56;
              } else {
                throw MOD_NAME + '.parse() invalid character encountered:' + _char + ' at index ' + idx;
              }
            }
    ...
    value: function getString(start, len) {
      ...
      if (bitIdx === 0) {
        ...
        while (byteIdx < _endIdx) {
          var _byte3 = this._getByte(byteIdx++);
          result += AIS_CHR_TBL[chrIdx | (_byte3 & hiMask) >> 6 - bitIdx];
          chrIdx = (_byte3 & loMask) << bitIdx;
        }
      }
      return result;
    }
    ...
    key: "getBytes",
    value: function getBytes(start, length) {
      if (start + length > this._bits || start < 0 || length < 0) {
        throw new Error(MOD_NAME + ".getBytes() invalid indexes: start=".concat(start, ", length=").concat(length, ", totalBits=").concat(this._bits));
      }
      var resultLength = Math.ceil(length / 8);
      var result = new Uint8Array(resultLength);
      for (var i = 0; i < resultLength; i++) {
        var bitStart = start + i * 8;
        var bitsLeft = length - i * 8;
        var bitsToRead = bitsLeft >= 8 ? 8 : bitsLeft;
